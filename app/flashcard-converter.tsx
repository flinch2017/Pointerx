...
import {
  ...
  MAX_PDF_BYTES,
  uploadPdfForFlashcards,
} from '@/lib/flashcard-api';
import { setLatestFlashcards } from '@/lib/flashcard-store';
import * as DocumentPicker from 'expo-document-picker';
import { router } from 'expo-router';
import { useEffect, useState } from 'react';
import { Pressable, ScrollView, StyleSheet, Text, View } from 'react-native';

export default function FlashcardConverterScreen() {
  ...
  useEffect(() => {
    const pendingSource = consumePendingChatMaterialSource('flashcards');
  ...
  async function generateFromPendingSource(source: PendingChatMaterialSource) {
    ...
    setStatus(
      source.sourceUrl
        ? 'Preparing flashcards from link...'
        : source.sourceFile
          ? 'Preparing flashcards from PDF...'
          : 'Preparing flashcards from chat...',
    );
    setIsGenerating(true);

    try {
      const flashcards = source.sourceUrl
        ? await generateFlashcardsFromUrl(
        ...
        : source.sourceFile
          ? await uploadPdfForFlashcards(source.sourceFile, (progress) => {
              setProgressPercent(progress.percent);
              setStatus(progress.status);
            })
        : source.sourceText
      ...
      if (!flashcards) {
        throw new Error('Pointerx could not find a source to create flashcards.');
      }

      setLatestFlashcards(flashcards);
      setProgressPercent(100);
      setStatus('Flashcards ready.');
      router.push('/flashcard-result');
    } catch (requestError) {
      const message =
        requestError instanceof Error
          ? requestError.message
          : 'Could not create flashcards from this source.';
  ...
  async function selectPdfFile() {
    ...
    try {
      const flashcards = await uploadPdfForFlashcards(asset, (progress) => {
        setProgressPercent(progress.percent);
        setStatus(progress.status);
      });
      ...
      const message =
        requestError instanceof Error
          ? requestError.message
          : 'Could not convert this PDF to flashcards.';
  ...
  return (
    <ScrollView
      ...
      <View style={styles.header}>
        <Text style={styles.kicker}>Convert PDF to Flashcards</Text>
        <Text style={styles.title}>
          {isGenerating ? 'Creating flashcards.' : 'Select a PDF file.'}
        </Text>
        <Text style={styles.subtitle}>
          Pointerx will extract the text, create active-recall flashcards, then send you to
      ...
      <Pressable
        ...
        <View style={styles.selectIcon}>
          {isGenerating ? (
            ...
            <IconSymbol name="tray.and.arrow.up.fill" color={palette.success} size={30} />
          )}
        </View>
